...
async function runQuickTests() {
  ...
  const tests = [
    ...
    },
    {
      name: '获取商品分类树',
      method: 'GET',
      url: `${BASE_URL}${API_BASE}/products/categories/tree`,
    ...
    },
    {
      name: '获取商品标签列表',
      method: 'GET',
      url: `${BASE_URL}${API_BASE}/products/tags/all`,
      token: testTokens.normalUser,
      expectedStatus: 200
    },
    {
      name: '获取商品分类列表',
      method: 'GET',
      url: `${BASE_URL}${API_BASE}/products/categories?level=1&page=1&perPage=10`,
      token: testTokens.normalUser,
      expectedStatus: 200
    },
    {
      name: '获取商品标签分页列表',
      method: 'GET',
      url: `${BASE_URL}${API_BASE}/products/tags?page=1&perPage=10`,
      token: testTokens.normalUser,
      expectedStatus: 200
    }
  ];
  ...
  for (let i = 0; i < tests.length; i++) {
    ...
    try {
      const req = request(BASE_URL)[test.method.toLowerCase()](test.url);
...
async function main() {
  try {
    console.log('🚀 产品模块API快速验证工具 (HTTP版本)\n');
    ...
    try {
      ...
      console.log('❌ 服务器未运行或无法连接，请先启动服务器: npm run dev');
      process.exit(1);
    }
